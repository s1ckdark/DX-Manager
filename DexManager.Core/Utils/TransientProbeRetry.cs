...
namespace DexManager.Utils
{
    ...
    /// 도는 경로이므로 후보 하나를 붙들고 늘어지지 않는다: 후보 하나당
    /// 최대 시도 <see cref="MaxAttempts"/>번, 그 사이 짧은 간격
    /// (<see cref="Gap"/>)만 둔다. 시도 횟수를 늘리면 "정말로 죽은"
    /// 후보를 판정하는 데 걸리는 시간이 그만큼 배로 늘어 시작을 눈에 띄게
    /// 늦추므로, 부하 스파이크를 넘기기에 충분하되 과하지 않은 값을 고른다.
    ///
    /// <para>
    /// 이 <b>횟수</b> 상한은 후보 하나에 대해서만 상한이다 - 후보를
    /// 순서대로 프로브하는 체인 전체로 보면 재시도가 후보 수만큼 곱해져
    /// 시작 지연이 배로 늘어난다. 체인 수준의 상한은 호출자가 넘기는
    /// <see cref="ProbeRetryBudget"/>이 따로 맡는다. 이 파일에서
    /// "예산"이라는 말은 <b>오직 그 시간 예산</b>만 가리킨다 - 위의
    /// <see cref="MaxAttempts"/>는 예산이 아니라 횟수 상한이다.
    /// 예산이 남아 있지 않으면 <b>재시도만</b> 건너뛴다. 각 후보의 첫
    /// 시도는 예산과 무관하게 언제나 실행하므로, 앞 후보가 느렸다는
    /// 이유로 살아 있는 뒤쪽 후보를 못 찾는 일은 생기지 않는다.
    /// </para>
    ///
