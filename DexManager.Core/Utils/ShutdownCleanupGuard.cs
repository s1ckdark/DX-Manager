...
{
    /// <summary>
    /// 정상 종료(GUI의 Avalonia Exit 이벤트, TUI의 정상 반환 경로)와
    /// 비정상 종료(SIGTERM/SIGINT/SIGHUP, 각 프론트엔드의 Program.cs 참고)가
    /// 같은 정리 로직을 정확히 한 번만 실행하도록 보장하는 가드.
    /// Avalonia나 콘솔 어느 쪽에도 의존하지 않으므로 헤드리스로 테스트할
    /// 수 있고, GUI(DexManager.Desktop)와 TUI(DexManager.Mac)가 함께 쓴다.
    ///
    /// <see cref="System.Threading.Interlocked.Exchange(ref int, int)"/> 하나로
    /// 승자를 가른다 - 두 경로가 서로 다른 스레드에서 동시에 들어와도(신호는
    /// 스레드풀 스레드에서, Exit/정상 반환은 다른 스레드에서 올 수 있다)
    /// 정확히 하나만 <paramref name="cleanup"/>을 실행한다.
    /// </summary>
