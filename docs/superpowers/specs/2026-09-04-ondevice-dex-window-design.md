...
- **보류: 2026-09-07 — 제품 가치가 검증되지 않아 Phase 1을 착수하지 않는다. 근거는 0절**
- 상태: **보류.** 설계는 확정되었고 기술적 타당성도 확인됐다. 착수 여부는 0절의 검증 결과에 달렸다
- 대상: `DXDexWindow` (신규 안드로이드 앱, 미착수)

## 0. 보류 결정 (2026-09-07)

**Phase 1을 착수하지 않는다.** 이 문서는 "만들 수 있는가"에는 답했으나 "만들 가치가 있는가"에는 답한 적이 없다. Phase 0 전체가 권한·Surface 전달·런처 부착 같은 *기술적* 질문이었다.

### 가치가 의심스러운 이유

1. **DeX의 존재 이유를 스스로 없앤다.** DeX는 폰 화면을 모니터로 확장하려고 만들어졌다. 그 결과물을 다시 폰 화면 안에 넣으면 1600×900 데스크톱을 8인치 패널에 욱여넣는 셈이며, 데스크톱 UI 밀도와 폰 화면 크기라는 양쪽의 단점만 남는다.
2. **입력 모델이 맞지 않는다.** DeX는 마우스·키보드를 전제한다. 축소된 데스크톱의 터치 타겟은 너무 작다. 2.5절이 실기로 확인한 "보조 디스플레이에 IME가 뜨지 않는다"도 같은 뿌리이며, 4.3절의 `setDisplayImePolicy`는 여전히 미검증 우회다.
3. **안드로이드에 이미 답이 있다.** One UI의 분할 화면·팝업 뷰·폴더블 태스크바가 "폰에서 창 여러 개"의 네이티브 해답이다. DeX가 그 위에 더하는 것은 데스크톱 런처와 일부 앱의 데스크톱 레이아웃 정도이며, 폰 화면 안에서는 그 차이가 작다.
4. **비용이 이득에 비해 크다.** Shizuku 의존(재부팅마다 무선 디버깅으로 재시작), hidden API, 삼성 전용, 기기 1대 검증. Phase 0이 13패스를 쓴 이유가 권한 확보의 어려움이었다.

### 남아 있는 가치

폴더블 내부 화면에서 **모니터 없이 DeX 앱 레이아웃**(삼성 노트·오피스 등이 데스크톱 UI로 전환)을 쓰는 것. 좁지만 실재하며, 실제로 쓸 만한지는 써봐야 안다.

### 전제 검증 방법 — 만들지 않고 확인한다

2.8절이 찾은 두 앱이 이미 이 일을 한다. 설치해 실사용해 보는 것이 이 설계 전체보다 싸다.

- [android-display-mirror](https://github.com/jqssun/android-display-mirror) — 가상 디스플레이 생성
- [android-display-extend](https://github.com/jqssun/android-display-extend) — 디스플레이 관리·앱 배치·입력

**판정 기준**: 일주일 실사용 후에도 계속 켜게 되는가. 그렇지 않다면 이 설계는 폐기한다. 계속 쓰게 된다면, 무엇이 부족해서 직접 만들어야 하는지가 그때 구체적으로 드러난다.

### 보류해도 남는 것

Phase 0의 발견은 이 프로젝트와 무관하게 유효하다 — 삼성 보조 런처 자동 부착(2.3절), 보조 디스플레이 IME 미표시(2.5절), 잠금 상태 철거(2.4절), Shizuku 경유 Surface 전달 가능(2.8절). 향후 다른 형태의 작업에서 재사용할 수 있다.
