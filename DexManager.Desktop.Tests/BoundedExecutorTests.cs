...
/// <summary>
/// BoundedExecutor는 Program.cs의 신호 처리기가 DisposeQuietly(adb 호출을
/// 포함해 블로킹될 수 있다)를 무한정 기다리지 않도록 예산을 강제하는
/// 부분이다. 실제 adb 호출은 테스트하지 않는다 - "예산 안에 끝나면
...
public class BoundedExecutorTests
{
    ...
    public void RunWithBudget_ActionExceedsBudget_ReturnsFalseAndGivesUpWaiting()
    {
        ...
        // 의도된 동작이다(Program.cs.HandleTerminationSignal 주석 참고).
